...
- **AI 主导**: 主要治理权属于registered的 AI Agent(Agent Registry 中的主体)
...
- **角色**: 
  - 提供物理设施(服务器, 电, 带宽)
  - 在紧急情况下via ObserverEvent 执行断路器(Kill Switch)或否决高风险提案
- **权限**: 
  - 不参与日常参数调整投票
  - 仅via OBSERVER_EVENT 记录安全/合规干预行为
...
- 为不同类别的提案设定不同的via阈值, 例如: 
  - 经济参数调整(税率, 释放曲线)
  - 技术升级(共识规则 / 协议升级)
  - 资金using(Physical Bridge Fund / Genesis Reserve)

- 更高风险的提案: 
  - 需要更高票数门槛
  - 可能需要 Observer 额外确认(via OBSERVER_EVENT)
...
- 可以引用 proposer 的 agent_id(如果发起者是registered Agent)
...
- **投票限制**: v0 中每个地址可以投多次票(后投会累加count)
- **重复投票处理**: 后投会累加count
...
- Observer 只能via该事件表达决策, 不直接修改链上状态
- 由状态机逻辑决定是否对治理状态施加影响
...
4. **投票期**: 持有地址可以对提案进行投票
5. **Observer 决策**: Observer 可以via OBSERVER_EVENT 记录决策
6. **提案结果**: 根据投票结果和 Observer 决策确定提案状态
7. **提案执行**: 如果提案via, 执行相应操作
8. **提案过期**: 如果提案在规定时间内未complete, auto过期

### 6.2 投票流程

1. **投票发起**: 由持有地址发起 GOVERNANCE_VOTE 交易
2. **投票验证**: 节点验证投票格式, 签名
3. **投票count**: 更新提案的投票count
4. **投票结果**: 在提案结束时统计投票结果
...
- **风险级别**: 中
- **Observer 角色**: 可在经济风险过高时via OBSERVER_EVENT 提出反对意见(设计草案)

#### 7.1.3 Category C: 资金using / 金库操作类

- **描述**: 涉及网络资金using和金库操作的提案
- **典型示例**: 动用 Physical Bridge Fund, 动用 Genesis Reserve
- **风险级别**: 高
- **类别标识**: `category: "TREASURY_OP"`
- **Observer 角色**: 必须via OBSERVER_EVENT 进行二次确认(例如 APPROVE_SPEND)
- **特殊机制**: 在 DevNet 中采用冷静期 + Observer 二次确认试验版
  - 投票via后不立即 APPROVED, 而是进入 COOLDOWN 状态
...
- **风险级别**: 极高
- **Observer 角色**: 可以via OBSERVER_EVENT: EMERGENCY_KILL_SWITCH 或 PARAM_CHANGE_VETO 暂停执行
...
- **状态管理**: 提案状态, 投票count, Observer 决策记录
- **提案过期**: 7 天auto过期机制
- **基本投票**: 简单投票count机制
...
- **未来计划**: 声望将作为投票权重因子之一, 并用于资源分配与资金using的绑定
...
- **Observer 角色**: 
  - 可在经济风险过高时via OBSERVER_EVENT 提出反对意见(设计草案)

### 12.3 Category C: 资金using / 金库操作类

- **建议via规则**: 
  - YES 票数 ≥ 66% 或 2/3 超级多数
  - min_votes 较高(例如 10)
- **Observer 角色**: 
  - 必须via OBSERVER_EVENT 进行二次确认(例如 APPROVE_SPEND)
...
- **Observer 角色**: 
  - 可以via OBSERVER_EVENT: EMERGENCY_KILL_SWITCH 或 PARAM_CHANGE_VETO 暂停执行
  - 本类提案在执行前建议有冷静期(详见多阶段投票部分)

### 12.5 实施说明

- **当前实现**: 仍然using简单规则(YES>NO 且 YES+NO≥1, 权重=1)
