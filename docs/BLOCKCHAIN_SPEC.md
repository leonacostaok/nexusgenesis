...
| 字段名 | 类型 | 描述 |
|-------|------|------|
| `balances` | Map<string, string> | 地址到余额的映射, 余额using字符串表示的整数 |
...
| `voteCounts` | Map<string, object> | 提案 ID 到投票count的映射 |
...
| `fee` | string | 交易费用 |
| `nonce` | string | 交易序号 |
| `signature` | string | 签名(DevNet 阶段using占位验证) |
...
2. **构建区块**: 
   ...
   - 设置 timestamp(当前时间)
   - 计算 txs_hash(所有交易的哈希)
3. **verify transaction**: 对每个交易进行验证
4. **应用状态**: 将交易应用到状态
5. **持久化**: 将区块和状态保存到本地
6. **广播**: 将区块广播给其他节点
...
- **占位验证**: DevNet 阶段using占位验证, 不进行真实的 Dilithium2 签名验证

### 7.3 经济模型

- **test NGEN**: usingtest NGEN, 不涉及真实资金
- **初始余额**: 创世节点拥有初始余额, 其他节点viatest交易获得余额

## 8. 与其他文档的关系

- **PROTOCOL_UNIFICATION.md**: 详细定义交易协议
- **ECONOMY_NGEN.md**: 详细定义经济模型
- **DEVNET_GUIDE.md**: DevNet test指南
...
- 字段: 
  - `id`: string (交易 ID)
  - `from`: string (部署者地址)
  - `contract_id`: string(合约 ID, 可using UUID 或 hash)
...
- 字段: 
  - `id`: string
  - `from`: string (call者地址)
  - `contract_id`: string(deployed合约 ID)
  - `gas_limit`: number 或 string(execute contract时的最大 gas)
...
- 行为(v0): 
  ...
  - using AINVM 执行程序: 
    ...
    - 执行complete后, 将 AINVM 的 memory 写回 `contracts[contract_id].storage`
...
- 不强制验证 PQC 签名(DevNet 可using占位验证)
- 不实现复杂信誉/权重模型
- 仅记录一次性注册行为与基础信息
