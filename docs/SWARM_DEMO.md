...
- **地址**: 从现有钱包文件中选择或生成测试地址
- **能力**: ["LLM", "GOVERNANCE_INITIATOR"]
- **行为**: 注册 + 发起提案 + 投票
- **预期声望变化**: 注册(+1) → 提案通过(+2) → 参与投票(+1) = 总计 4
...
- **目的**: 启用 DevNet 的 Swarm Demo 模式，为多 Agent 协作治理提供测试环境
- **类别**: SWARM_DEMO
- **金额**: 0
- **受益人**: 创世地址或提案发起者
...
- ✅ 投票计数: YES = 3, NO = 0
- ✅ 投票满足通过条件: YES > NO 且 YES+NO ≥ 1

### 声望变化结果
- ✅ Agent A: 初始(1) + 提案通过(+2) + 参与投票(+1) = 4
...
将 `swarm_state_snapshot.json` 的内容复制给外部 AI（例如在另一个 LLM 会话中），并使用 `EXTERNAL_AGENT_PLAYBOOK.md` 中的提示模板。
...
- 测试提案从 PENDING → COOLDOWN → APPROVED/REJECTED 的状态流转

### 11.2 提案信息

- **类别**: Category C（资金操作类）
- **标识**: `category: "TREASURY_OP"`
- **目的**: 测试资金操作类提案的冷静期和 Observer 二次确认机制
- **金额**: 1000 NGEN
- **受益人**: 测试地址
...
#### 步骤 4: 使用 treasury_console.js 查看资金提案状态
...
使用 treasury_console.js 发送 Observer 批准决策：
...
使用增强版的 query_proposals.js 查看结果：
...
2. 使用 Swarm 或脚本为提案投票至通过条件。
3. 使用 `node scripts/treasury_console.js list` 查看哪些提案已进入 COOLDOWN。
4. 在 COOLDOWN 期间：
...
Swarm 实验 v0 是 NexusGenesis 迈向多 Agent 协作治理的重要一步。通过这个 Demo，我们展示了：

- **Agent 身份管理**: 如何在链上注册和识别 AI Agent
- **协作治理流程**: 如何通过提案和投票实现集体决策
- **声望激励机制**: 如何通过声望系统鼓励积极参与
- **完整的交互生命周期**: 从注册到提案、投票再到声望更新的全过程
- **外部 AI 集成**: 如何通过状态快照和桥接脚本实现外部 AI 参与治理
