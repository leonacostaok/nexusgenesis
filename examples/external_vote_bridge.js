/**
 * External Vote Bridge Script
 * 
 * 功能：
 * 1. 读取外部 AI Agent 生成的投票决策 JSON 文件
 * 2. 验证决策格式和字段
 * 3. 检查 Agent 是否在注册表中
 * 4. 构造并发送 GOVERNANCE_VOTE 交易
 * 5. 输出执行结果
 */
...
const DEFAULT_DECISION_PATH = path.join(process.cwd(), 'external', 'decisions', 'vote_decision.json');
...
function isAgentRegistered(address) {
  console.log('Checking if agent is registered:', address);
  const state = readStateFile();
  console.log('State:', JSON.stringify(state, null, 2));
  const agentRegistry = getAgentRegistry(state);
  console.log('Agent Registry:', JSON.stringify(agentRegistry, null, 2));
  const addressIndex = agentRegistry.addressIndex || {};
  console.log('Address Index:', JSON.stringify(addressIndex, null, 2));
 ...
 */
function validateVoteDecision(decision) {
  // 检查 type 字段
  if (decision.type !== 'vote') {
    console.error('Error: Invalid decision type. Expected "vote".');
    return false;
  }
  ...
  // 检查 vote_option 字段
  if (!decision.vote_option) {
    console.error('Error: Missing required field "vote_option".');
    return false;
  }
  
  // 检查 vote_option 值
  const validVoteOptions = ['YES', 'NO', 'ABSTAIN'];
  if (!validVoteOptions.includes(decision.vote_option)) {
    console.error(`Error: Invalid vote_option. Expected one of: ${validVoteOptions.join(', ')}`);
    return false;
  }
 ...
 * @returns {object} 交易对象
 */
function createVoteTransaction(decision) {
  return {
    id: `governance-vote-${decision.from_address.slice(-8)}-${Date.now()}`,
    tx_type: 'GOVERNANCE_VOTE',
    from: decision.from_address,
    to: decision.from_address, // 投票交易的 to 字段可以是任意地址
    amount: '1',
    fee: '100',
    timestamp: Date.now(),
    nonce: '1',
    ...
    voter_id: decision.from_address,
    vote_option: decision.vote_option,
    timestamp: Date.now(),
    reason: decision.justification || 'External AI Agent vote'
  },
 ...
 */
async function main() {
  console.log('=== External Vote Bridge ===\n');
  ...
  try {
    ...
    // 验证决策格式
    console.log('Step 2: Validating vote decision format');
    if (!validateVoteDecision(decision)) {
    ...
    console.log('Checking address:', decision.from_address);
    ...
    // 构造交易
    console.log('Step 4: Creating vote transaction');
    const transaction = createVoteTransaction(decision);
    console.log(`✅ Transaction created: ${transaction.id}`);
    console.log('Transaction:', JSON.stringify(transaction, null, 2));
    
    // 发送交易
    console.log('Step 5: Sending transaction to injection interface');
    console.log('URL:', TX_INJECTION_URL);
    ...
    if (result.success) {
      ...
      console.log(`Vote: ${decision.vote_option}`);
      console.log(`Justification: ${decision.justification || 'No justification provided'}`);
    } else {
