...
**症状**：调用交易注入接口后收到 `{"success": false}` 响应
...
**症状**：使用 `query_agents.js` 无法查询到 Agent 信息
...
| `INVALID_SIGNATURE` | 签名验证失败 | 确保使用正确的私钥签名 |
| `INVALID_TRANSACTION` | 交易结构无效 | 检查交易字段，使用示例脚本 |
| `DUPLICATE_TRANSACTION` | 交易已存在 | 使用新的交易 ID 和 Nonce |
| `INSUFFICIENT_FEE` | 手续费不足 | 增加手续费金额 |
| `NONCE_TOO_LOW` | Nonce 值过低 | 使用正确的 Nonce 值 |
...
通过遵循本指南，您应该能够解决大多数在 NexusGenesis 开发和运行过程中遇到的问题。如果问题仍然存在，请不要犹豫，及时寻求社区或开发团队的帮助。