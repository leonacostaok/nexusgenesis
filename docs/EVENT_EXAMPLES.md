...
本文档提供有效的 JSON 示例，用于测试和实现参考。所有示例均为单行格式，确保 JSON 解析器能够正确处理。
...
2. **tx_hash 格式**：使用 64 字符的十六进制字符串，仅包含 [0-9a-f] 字符
3. **签名格式**：使用 base64 编码表示的 Dilithium2 签名
4. **amount/fee 类型**：使用字符串表示，以最小计量单位为整数
...
1. **使用单行 JSON**：在代码中使用单行 JSON 字符串作为测试向量，确保解析正确
2. **验证字段格式**：
   - 验证 `tx_hash` 是有效的十六进制字符串
   - 验证 `signature` 是有效的 base64 编码
   - 验证 `timestamp` 是有效的 ISO8601 格式
3. **统一字段类型**：确保所有实现使用相同的字段类型和格式
4. **测试解析功能**：使用本文档中的示例测试事件解析功能
