...
| `observer_id` | string (地址) | 是 | 执行操作的观察者地址 |
| `tx_hash` | string | 否 | 关联的链上交易哈希，格式为 64 字符的十六进制字符串（仅包含 0-9 和 a-f） |
| `signature` | string | 否 | 观察者对事件的签名，使用 Dilithium2 算法，以 base64 编码表示 |
...
- **验证机制**：
  - 观察者签名 `signature` 必须通过验证
  - 只有授权的观察者地址才能发起 `OBSERVER_EVENT` 交易
...
- **主键设计**：
  - 使用 `proposal_id` + `proposer_id` + `timestamp` 作为复合主键
...
- **与治理模块绑定**：
  - 提案上链后，自动触发治理投票流程
  - 投票交易通过 `proposal_id` 关联到具体提案
  - 投票结果通过另一种特殊交易类型 `VOTE_RESULT` 记录
...
| `amount` | string | 是 | 交易金额，以最小计量单位为整数，使用字符串表示，暂不支持小数点 |
| `fee` | string | 是 | 交易费用，以最小计量单位为整数，使用字符串表示，暂不支持小数点 |
| `timestamp` | string (ISO8601) | 是 | 交易时间戳，校验时以此为准 |
| `nonce` | string | 是 | 交易随机数，用于防止重放攻击 |
| `payload` | object | 是 | 交易负载，包含具体的事件或提案数据 |
| `signature` | string | 是 | 交易签名，使用 Dilithium2 算法，以 base64 编码表示 |

### 5.2 字段语义说明

- **from**：交易发起者，必须与签名者一致
- **to**：逻辑上的接收方，对于治理交易，建议使用 Genesis 地址作为治理模块/金库的代表地址
- **timestamp**：交易的时间戳，payload 内的 timestamp 应与交易级别的 timestamp 保持一致，校验时以交易级别的 timestamp 为准
- **amount/fee**：使用字符串表示，以最小计量单位为整数，暂不支持小数点
- **signature**：使用 Dilithium2 算法生成的签名，以 base64 编码表示
...
- **防重放**：使用 `event_id` 和 `timestamp` 防止重放攻击
- **数据完整性**：链上交易必须包含足够的字段以确保数据完整性

## 8. 实现建议

- **链下存储**：对于较长的文本字段，建议使用 IPFS 等分布式存储系统
