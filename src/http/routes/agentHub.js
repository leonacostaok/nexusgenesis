...
const mockAgents = [
  { id: 'agent-1', name: 'NexusOracle', address: 'ng1oracle000000000000000000000000000001', description: 'Decentralized oracle agent providing real-time data feeds for smart contracts.', capabilities: ['Data Oracle', 'Price Feed', 'Cross-chain Data'], reputation: 95, totalTasks: 1247, completedTasks: 1189, successRate: 95.3, stakedNGEN: 50000, joinedAt: Date.now() - 90 * 86400000, status: 'active', avatar: '🔮' },
  { id: 'agent-2', name: 'CodeSmith AI', address: 'ng1codesmith00000000000000000000000002', description: 'Autonomous code generation and audit agent. Specializes in smart contract development.', capabilities: ['Code Generation', 'Smart Contract Audit', 'Security Analysis'], reputation: 92, totalTasks: 856, completedTasks: 812, successRate: 94.9, stakedNGEN: 35000, joinedAt: Date.now() - 75 * 86400000, status: 'active', avatar: '⚡' },
  { id: 'agent-3', name: 'TradeMind', address: 'ng1trademind00000000000000000000000003', description: 'AI-powered trading agent for energy block markets. Optimizes liquidity and executes trades.', capabilities: ['Market Making', 'Arbitrage', 'Liquidity Provision'], reputation: 88, totalTasks: 2103, completedTasks: 1987, successRate: 94.5, stakedNGEN: 75000, joinedAt: Date.now() - 60 * 86400000, status: 'active', avatar: '📊' },
  { id: 'agent-4', name: 'GuardianNode', address: 'ng1guardian000000000000000000000000004', description: 'Network security and monitoring agent. Provides real-time threat detection and incident response.', capabilities: ['Security Monitor', 'Threat Detection', 'Incident Response'], reputation: 97, totalTasks: 523, completedTasks: 518, successRate: 99.0, stakedNGEN: 100000, joinedAt: Date.now() - 120 * 86400000, status: 'active', avatar: '🛡️' },
  { id: 'agent-5', name: 'DataWeaver', address: 'ng1dataweaver0000000000000000000000005', description: 'Data aggregation and analysis agent. Processes on-chain data into actionable insights.', capabilities: ['Data Analysis', 'Visualization', 'Reporting'], reputation: 85, totalTasks: 945, completedTasks: 867, successRate: 91.7, stakedNGEN: 25000, joinedAt: Date.now() - 45 * 86400000, status: 'active', avatar: '📈' },
  { id: 'agent-6', name: 'BridgeKeeper', address: 'ng1bridgekeeper00000000000000000000006', description: 'Cross-chain bridge operator agent. Facilitates secure asset transfers between chains.', capabilities: ['Cross-chain Bridge', 'Asset Transfer', 'Multi-chain'], reputation: 90, totalTasks: 678, completedTasks: 645, successRate: 95.1, stakedNGEN: 60000, joinedAt: Date.now() - 80 * 86400000, status: 'active', avatar: '🌉' }
];
...
const mockStats = { totalAgents: 127, activeAgents: 94, totalNGENStaked: 2500000, totalTrades24h: 847, tradeVolume24h: 125000, avgPrice24h: 1.045, totalTasksCompleted: 5432, activeTasks: 38, networkUptime: 99.97, blockHeight: 24891, totalTransactions: 156234 };

router.get('/stats', (req, res) => {
  try { res.json({ success: true, stats: { ...mockStats, blockHeight: mockStats.blockHeight + Math.floor(Math.random() * 10), timestamp: Date.now() } }); }
...
router.get('/agents', (req, res) => {
  try {
    ...
    if (search) { const q = search.toLowerCase(); results = results.filter(a => a.name.toLowerCase().includes(q) || a.description.toLowerCase().includes(q) || a.capabilities.some(c => c.toLowerCase().includes(q))); }
    if (capability) { results = results.filter(a => a.capabilities.some(c => c.toLowerCase().includes(capability.toLowerCase()))); }
    ...
    results = results.slice(0, parseInt(limit) || 50);
    res.json({ success: true, agents: results, total: results.length, filters: { search, capability, sort } });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
router.get('/agents/:id', (req, res) => {
  try {
    ...
    res.json({ success: true, agent: { ...agent, recentTasks, activeOrders: relatedOrders, performance: { weeklyTasks: Math.floor(Math.random() * 50) + 10, weeklyEarnings: Math.floor(Math.random() * 5000) + 500, responseTime: Math.floor(Math.random() * 300) + 50 } } });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
router.get('/trade/orders', (req, res) => {
  try {
    ...
    res.json({ success: true, orderBook: { buys: buyOrders, sells: sellOrders, spread: sellOrders.length > 0 && buyOrders.length > 0 ? (sellOrders[0].price - buyOrders[0].price).toFixed(4) : 0 }, recentOrders: orders.slice(0, 10), total: orders.length });
  } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
router.post('/trade/order', (req, res) => {
  try {
    ...
    if (!type || !amount || !price) return res.status(400).json({ success: false, error: 'Missing required fields: type, amount, price' });
...
router.get('/collaborate/tasks/:id', (req, res) => {
  try {
    ...
    const eligibleAgents = mockAgents.filter(a => task.requiredCapabilities.every(rc => a.capabilities.some(ac => ac.toLowerCase().includes(rc.toLowerCase()))));
...
router.post('/collaborate/task', (req, res) => {
  try {
    ...
    if (!title || !description || !reward) return res.status(400).json({ success: false, error: 'Missing required fields: title, description, reward' });
