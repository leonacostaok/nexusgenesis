...
- 允许运行在外部环境(如 TRAE 或其他 LLM 宿主)中的 AI Agent: 
  - 读取链上状态摘要; 
  - 做出提案/投票决策; 
  - via文件或标准化 JSON 的方式, 把决策"交给" NexusGenesis 仓库中的桥接脚本执行上链. 

- 提供一个安全, 简单的接入通道, 避免修改核心共识, 经济和治理逻辑
- 仅在 DevNet / 本地环境中using, 不涉及真实资金和互联网连接
...
- `from_address`: registered Agent 的 ng1 地址
- `proposal_id`: 目标提案 ID
...
- `proposal_id`: 提案 ID
- `purpose`: 提案目的
- `category`: 提案类别, 建议using "SWARM_DEMO"
- `amount`: 提案金额, 建议为 "0"
- `beneficiary`: 受益人地址
...
1. **读取链上状态摘要**: 
   ...
   - 或运行 `scripts/query_chain.js` 和 `scripts/query_proposals.js` get status
...
3. **构造治理交易**: 
   - 根据决策类型构造对应的交易
   - 填充必要的交易字段
   - usingtest签名(DevNet 环境)

4. **send transaction**: 
   - via本地 HTTP 注入接口send transaction
   - 记录发送结果
...
- **不处理真实资金**: `amount` 建议为 "0" 或极小test值
- **仅允许实验类别**: 仅允许 `category` 为 "SWARM_DEMO" 或其他明确标记为教学/实验用途的类别
- **不影响生产参数**: 不允许外部 AI 提案/投票对生产参数或真实资金路径造成影响
- **仅本地using**: 不接入互联网, 仅在 DevNet / 本地环境中using

### 4.2 功能限制

- **签名验证**: DevNet 环境usingtest签名, 不进行真实的 Dilithium 签名验证
...
- **验证 Agent 注册状态**: 确保 `from_address` 对应registered的 Agent
...
- **不暴露私钥**: 桥接脚本不应处理或存储私钥
- **usingtest签名**: 在 DevNet 环境中usingtest签名
- **日志脱敏**: 确保日志中不包含敏感信息
...
- **不联网**: 不与外部网络交互
- **状态隔离**: using独立的状态目录, 避免影响其他test

## 9. 未来扩展

### 9.1 功能扩展

- **支持更多决策类型**: 如agent投票, 批量投票等
- **增强状态摘要**: 提供更详细的链上状态信息
...
3. **提案does not exist**
   - 症状: 桥接脚本报告 `proposal_id` does not exist
   - 解决: 确保提案created, 可运行 `scripts/query_proposals.js` 检查

4. **HTTP 注入失败**
   - 症状: 桥接脚本报告无法连接到 HTTP 接口
   - 解决: 确保 Genesis 节点started并在端口 19890 监听
...
外部 AI Agent 接入规范(v0)为运行在外部环境的 AI Agent 提供了一个安全, 简单的接入通道, 使它们能够参与 NexusGenesis 的治理流程. via标准化的 JSON 格式和文件交互方式, 外部 Agent 可以读取链上状态, 做出决策, 并via桥接脚本执行上链操作. 
